...
typedef enum
{
    ...
    /* 返回：IsBoard2Valid(U8 WK_TRUE/WK_FALSE) PCB板1与板2的连接状态是否有效
        + eSensors(U8[emSensorId_End] emSensorStatus) 各传感器状态 */
    ...
    + LedSelect(U8) + IsTri(U8) + BrightLevel(U16) + TriTime(U16) */
    EnumBoardId_getBrightTaskInfo = 6, // 获取光源模块任务信息

    /*****************************************************玻片盒翻转模块*************************************************/
    /* 请求： eTskType(U8 emTskDUpenderType) + uTskId(U8)*/
    /* 返回： */
    EnumBoardId_setUpenderTask = 7,//设置玻片盒翻转任务

    /* 请求：*/
    /* 返回：eTskType(U8 emTskDUpenderType) + uTskId(U8) + eStatus(U8 emWorkStatus) + eSelute(U8 emSeluteDUpender) + eCurPos(U8 emPositionType)+ m_iIsMove(U8) */
    ...
    /* 请求： */
    /* 返回：eTskType(U8 emTskDQRScannerType) + uScannerId(U8) + eStatus(U8 emWorkStatus) + eSelute(U8 emSeluteDQRScanner) + Connect(U8[128]) */
    ...
    + IsClamp(U8 WK_TRUE/WK_FALSE) 夹爪当前方向+ uTskStep(U8)+ nowStep(U8) + TotalLength(U16)+ IsEdgeCheck(U8)+eGripperSensor(U8 m_eGripperSensor) */
    ...
    /* 返回：eTskType(U8 emTskDBeltInType)  + uTskId(U8) + uIsScan(U8) + eStatus(U8 emWorkStatus) + eSelute(U8 emSeluteDBelt)
        ...
        + QRScanContent(U8[128]) 二维码扫描内容*/
    EnumBoardId_getBeltInTaskInfo = 14,//获取入舱皮带任务状态

    /*****************************************************入舱推杆模块*************************************************/
    /* 请求： eTskType(U8 emTskDPoleInType) + uTskId(U8)*/
    /* 返回： */
    EnumBoardId_setPoleInTask = 15,//设置入舱推杆任务

    /* 请求：*/
    /* 返回：eTskType(U8 emTskDPoleInType) + uTskId(U8) + eStatus(U8 emWorkStatus) + eSelute(U8 emSeluteDPole) + eCurPos(U8 emPositionType) */
    EnumBoardId_getPoleInTaskInfo = 16,//获取入舱推杆任务状态

    /*****************************************************出舱推杆模块*************************************************/
    /* 请求： eTskType(U8 emTskDPoleOutType) + uTskId(U8)*/
    /* 返回： */
    EnumBoardId_setPoleOutTask = 17,//设置出舱推杆任务

    /* 请求：*/
    /* 返回：eTskType(U8 emTskDPoleOutType) + uTskId(U8) + eStatus(U8 emWorkStatus) + eSelute(U8 emSeluteDPole) + eCurPos(U8 emPositionType) + eBoxNum(U8)*/
    ...
    /*****************************************************配置信息模块**************************************************/
    /******** ABC层配置参数指令 *********/
    /* 请求：uCfgId(U16) + uCfgLen(U16) + cfgData(U8[N]) */
    ...
    /* 请求：uCfgId(U16)    */
    /* 返回：uCfgId(U16) +    bIsSucceed(U8) + uCfgLen(U16) + cfgData(U8[N])    */
    ...
    /* 返回：id(U16)+totalNum(U16)+startId(U16)+num(U8)+{pos1(I16)+pos2(I16) */
    EnumBoardId_getXYZReady = 36,//获取移动轨迹数据
    ...
    /* 请求：uPeriod(U16) +speedZup(U16) +speedZdown(U16)+ uCycleDelay(U8) + uPercent(U8) + uSpdFilt(U8)+ uErrUP(U8)+ uErrDown(U8)*/
    /* 返回：*/
    EnumBoardId_setFollowParam = 40, // 设置跟随移动运动控制相关参数

    /* 请求： */
    /* 返回：uPeriod(U16) +speedZup(U16) +speedZdown(U16)+ uCycleDelay(U8) + uPercent(U8) + uErrCount(U8)+ uErrCountMin(U8)*/
    ...
    /* 请求：*/
    /* 返回： eTskType(U8 emTskDRotateType) + uTskId(U8)+iStep(i32) + eStatus(U8 emWorkStatus) + eSelute(U8 emSeluteDrotate)*/
    EnumBoardId_getRotateTaskInfo = 46,//获取夹爪翻转任务状态


    /* 请求：u16 start + u8 needNum*/
    /* 返回：u16 start + u8 num + xyz(U8 目标轴) + AimPos(I32目标位置) + xyzMoveTime(I32移动时间) + TotalNum(I16) + realStart(U16) + realNum(u16) + triPoses{I32,I32} */
    ...
    /* 返回：isOK(U8)+id(U16)+totalNum(U16)+{param(double)}*/
    EnumBoardId_setEquationParam = 52,//下发移动轨迹方程式系数

    /* 请求：mode(U8)(mode:0 获取移动 mode:1 获取跟随)*/
    /* 返回：isSuccess(U8) + mode(U8) + moveErr(U8) + maxErr(U32) + avgErr(I16) + rangErr(U8) + sumErr(I32) + errCnt(U32) + errNum(U8) + {err(I8) + cnt(U16)}*/
    EnumBoardId_getZAxisAdptErr = 53, //获取移动或跟随自适应误差结果

    /* 请求：isFollowTime(U8)(0 跟随轴 1 跟随时间) mode(U8)(0 正常跟随 1 直线跟随 2 正弦跟随)*/
    /* 返回：isSuccess(U8)*/
...
public:
    ...
    static QByteArray formatBoardCmd(quint8 cmdid, QByteArray cmdconetent);
